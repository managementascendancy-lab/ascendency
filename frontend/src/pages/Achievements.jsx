...
export default function Achievements() {
  ...
  return (
    <div className="py-14">
      <SEO title="Performance Achievements | Ascendancy" description="Unlock performance achievements as you type faster, more accurately and ascend the Ascendancy classifications." />

      <Reveal>
        <span className="tech-label text-gold-bright">PERFORMANCE MILESTONES</span>
      ...
      {!user && (
        <Reveal delay={80}>
          <div className="mt-6 border border-bronze/40 bg-navy-dark px-4 py-3 font-mono text-xs text-sage">
            {"> "}<Link to="/auth" className="text-gold-bright">AUTHENTICATE</Link> TO TRACK YOUR ACHIEVEMENTS.
          </div>
        </Reveal>
      )}
          ...
          return (
            <Reveal key={a.id} delay={(i % 3) * 80}>
              ...
              >
                ...
                <p className={`mt-1 font-body text-sm ${has ? "text-cream/60" : "text-cream/30"}`}>{a.desc}</p>
              </div>
            </Reveal>
          );
        })}
